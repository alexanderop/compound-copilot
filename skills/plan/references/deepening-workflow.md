...
- `cspecflow` for missing user flows, edge cases, and handoff gaps
- `cexplore` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `clearnings` for institutional knowledge and past solved problems
- `cdocs` for official framework or library behavior
- `cbestpractices` for current external patterns and industry guidance

**Key Technical Decisions**
- `architecture-reviewer` for design integrity, boundaries, and architectural tradeoffs
- Add `cdocs` or `cbestpractices` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `architecture-reviewer` for validating that the technical design accurately represents the intended approach and identifying gaps
- `cexplore` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `cbestpractices` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `cexplore` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `refactoring-reviewer` for consistency, duplication risks, and alignment with existing patterns
- Add `cspecflow` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `architecture-reviewer` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `cexplore` (Scope: `performance`) for scalability, latency, throughput, and resource-risk analysis
  - `security-reviewer` for auth, validation, exploit surfaces, and security boundary review
  - `cexplore` (Scope: `data integrity`) for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `security-reviewer` for security, auth, privacy, and exploit risk
  - `cexplore` (Scope: `data integrity`) for persistent data safety, constraints, and transaction boundaries
  - `cexplore` (Scope: `migration`) for migration realism, backfills, and production data transformation risk
  - `cexplore` (Scope: `deployment`) for rollout checklists, rollback planning, and launch verification
...
Artifact-backed mode uses a per-run scratch directory under `.context/plan/deepen/`.
...
1. **Summarize the agent and its target section** — e.g., "The architecture-reviewer reviewed Key Technical Decisions and found:"
...
3. **Ask the user** using `#askQuestions`:
